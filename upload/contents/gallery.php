...
# normale groesse anzeigen
  ...
  if ($allgAr['gallery_img_koms'] == 1) {
	...
	//$erg_cat = db_fetch_assoc($erg);
	if ( db_num_rows($erg) > 0 ) {
	
	$slide = db_query("SELECT * FROM prefix_gallery_imgs WHERE cat = $cid");
	while($rowslide = db_fetch_assoc($slide)){
		$bxslider .= "<li><center><img src=\"/include/images/gallery/img_".$rowslide['id'].".".$rowslide['endung']."\" /></center></li>";
	}

		
      $tpl->set('imgperline', $allgAr['gallery_imgs_per_line']);
	  $tpl->set('bxslider',$bxslider);
	  $tpl->set('cname', $cname);
